...
export default function ListUpcomingGameNightsComponent() {
  ...
  if (query.isSuccess) {
    console.log(query.data.gameNights);
  }
...
```javascript
useCreateGameNight(options?: useDataConnectMutationOptions<CreateGameNightData, FirebaseError, CreateGameNightVariables>): UseDataConnectMutationResult<CreateGameNightData, CreateGameNightVariables>;
```
You can also pass in a `DataConnect` instance to the Mutation hook function.
```javascript
useCreateGameNight(dc: DataConnect, options?: useDataConnectMutationOptions<CreateGameNightData, FirebaseError, CreateGameNightVariables>): UseDataConnectMutationResult<CreateGameNightData, CreateGameNightVariables>;
```
...
```
### Return Type
Recall that calling the `CreateGameNight` Mutation hook function returns a `UseMutationResult` object. This object holds the state of your Mutation, including whether the Mutation is loading, has completed, or has succeeded/failed, among other things.

To check the status of a Mutation, use the `UseMutationResult.status` field. You can also check for pending / success / error status using the `UseMutationResult.isPending`, `UseMutationResult.isSuccess`, and `UseMutationResult.isError` fields.

To execute the Mutation, call `UseMutationResult.mutate()`. This function executes the Mutation, but does not return the data from the Mutation.

To access the data returned by a Mutation, use the `UseMutationResult.data` field. The data for the `CreateGameNight` Mutation is of type `CreateGameNightData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `CreateGameNight`'s Mutation hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, CreateGameNightVariables } from '@dataconnect/generated';
import { useCreateGameNight } from '@dataconnect/generated/react'

export default function CreateGameNightComponent() {
  // Call the Mutation hook function to get a `UseMutationResult` object which holds the state of your Mutation.
  const mutation = useCreateGameNight();

  // You can also pass in a `DataConnect` instance to the Mutation hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const mutation = useCreateGameNight(dataConnect);

  // You can also pass in a `useDataConnectMutationOptions` object to the Mutation hook function.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  const mutation = useCreateGameNight(options);
  ...
  const mutation = useCreateGameNight(dataConnect, options);
  ...
  mutation.mutate(createGameNightVars, options);
  ...
  if (mutation.isSuccess) {
    console.log(mutation.data.gameNight_insert);
  }
...
export default function InviteToGameNightComponent() {
  ...
  // If the Mutation is successful, you can access the data returned using the `UseMutationResult.data` field.
  if (mutation.isSuccess) {
    console.log(mutation.data.gameNightInvitation_insert);
  }
  return <div>Mutation execution {mutation.isSuccess ? 'successful' : 'failed'}!</div>;
}
```
